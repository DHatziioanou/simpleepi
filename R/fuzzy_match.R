...
fuzzy_match <- function(lla, dba, stringency, pc_format, case_column, parallel){
  ...
  if (!(missing(case_column))) {
    ...
    # Format strings
    dbaob[ , c("db_Surname", "db_Name", "db_Postcode") := lapply(.SD, tidywords), .SDcols = c(dba$sn, dba$fn, dba$pc)]
    llaob[ , c("ll_Surname", "ll_Name", "ll_Postcode") := lapply(.SD, tidywords), .SDcols = c(lla$sn, lla$fn, lla$pc)]
    ...
    # Format DOB
    dbaob[,(dba$dob) := tidydates(dba$ob[, get(dba$dob)]), by = 1:nrow(dbaob)]
    llaob[,(lla$dob) := tidydates(lla$ob[, get(lla$dob)]), by = 1:nrow(llaob)]
    # 
